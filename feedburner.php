<?php
/*
Plugin Name: FeedBurner FeedSmith Reloaded
...
update_option('feedburner_settings',$data);
...
function fb_store_hash($generated_hash) {
	return update_option('feedsmith_token',$generated_hash);
}
...
function ol_add_feedburner_options_page() {
	if (function_exists('add_options_page')) {
		add_options_page('FeedBurner', 'FeedBurner', 'manage_options', basename(__FILE__), 'ol_feedburner_options_subpanel');
	}
}
...
function ol_feedburner_options_subpanel() {
	...
	if (ol_is_authorized()) {
		...
		<ol>
		<li>To get started, <a href="http://feedburner.google.com/fb/a/addfeed?sourceUrl=' . get_bloginfo('url') . '" target="_blank">create a FeedBurner feed for ' . get_bloginfo('name') . '</a>. This feed will handle all traffic for your posts.</li>
